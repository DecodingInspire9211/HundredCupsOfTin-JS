...
class Player extends BaseGameObj {
  ...
  ui = (uictx) => {
    ...
    this.profitdisplay = new Label(
      ...
      64,
      64,
      `Income: ${global.economy.getStats().income}\n\rExpenses: ${global.economy.getStats().expenses}`,
      16,
      "white",
      "right",
    );
    ...
    this.money = new Label(
      ...
      0,
      64,
      `${global.economy.getStats().currency.symbol} ${global.economy.getStats().money}`,
      16,
      "black",
      "left",
    );
  ...
  reactToTrigger = (triggeringObject: any) => {
    if (triggeringObject instanceof Coffeemachine) {
      ...
      if (global.handleInput.keyBinary & Key.Act) {
        ...
        global.handleInput.keyBinary & Key.Take &&
        triggeringObject.coffeeReady
      ) {
        if (triggeringObject.coffeeAtMachine > 0) {
          // Player can't take more than 3 coffee cups at once
          const coffeeToTake = Math.min(
            triggeringObject.coffeeAtMachine,
            3 - this.amountCoffee,
          );
    ...
    if (triggeringObject instanceof Customer) {
      ...
      if (
        global.handleInput.keyBinary & Key.Act &&
        this.hasCoffee &&
        !triggeringObject.hasCoffee
      ) {
