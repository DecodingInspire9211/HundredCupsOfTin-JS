...
export class GameWorld extends Scene {
    ...
    ui = (uictx: CanvasRenderingContext2D) => {
        ...
        global.uictx!.fillStyle = "beige";
        global.uictx!.fillRect(this.gap + 128, global.ui!.height - this.gap -64, 256+64, 64);
        global.uictx!.strokeStyle = "black";
        global.uictx!.lineWidth = 4;
        global.uictx!.strokeRect(this.gap + 128, global.ui!.height - this.gap -64, 256+64, 64);
    ...
    createObjects = () => {
        ...
        for(let x = 0; x < this.grid.tiles; x++) {
            ...
            let wall = new Wall(`Wall`, this.grid.x, this.grid.y - TILE_SIZE, TILE_SIZE, TILE_SIZE * 2, -1, undefined, true, false);
            this.sceneObjects.push(wall);
        }

        for(let x = 6; x < this.grid.tiles; x++) {
            this.grid.setPos(x, 0.5)
            let wallCounter = new WallCounter(`WallCounter`, this.grid.x, this.grid.y - (TILE_SIZE * 1.5), TILE_SIZE, TILE_SIZE*2.5, 0, true, false);
            this.sceneObjects.push(wallCounter);
        }

        for(let x = 6; x < this.grid.tiles; x++) {
            this.grid.setPos(x, 3)
            let pseudo = new Pseudo(`Pseudo`, this.grid.x, this.grid.y - ((TILE_SIZE / 2)-(TILE_SIZE/2)), TILE_SIZE, TILE_SIZE*0.5, 0, true, false);
            let counter = new Counter(`Counter`, this.grid.x, this.grid.y - (TILE_SIZE / 2), TILE_SIZE, TILE_SIZE*1.5, 2.5, false, false);
        ...
        let chair3 = new Chair(`Chair`, this.grid.x, this.grid.y - TILE_SIZE, TILE_SIZE, TILE_SIZE * 2, 4, 3, true, true);

        this.grid.setPos(4, 8);
        let chair4 = new Chair(`Chair`, this.grid.x, this.grid.y - TILE_SIZE, TILE_SIZE, TILE_SIZE * 2, 8, 1, true, true);
